...
- **Executor routing** — the `executor-router` agent's judgment call: send a task to the Codex CLI (via `codex-executor`) or to a Claude executor, and pick the Codex reasoning-effort level.
...
- **Second-model review** — the `codex-executor` agent (in `read-only` mode) running the Codex CLI read-only over the integration diff; its findings face the same finding verifier as the Claude personas (cross-model both ways).
- **Simplify-as-you-go** — the mid-run simplification hook: after a wave that merged 2+ tasks, consolidate the integration branch (Shepherd's own inline simplify doctrine) before the next wave forks from it.
...
- **Proof** — the browser-testing phase: an agent drives the agent-browser CLI (Shepherd's own inline proof doctrine) against the merged worktree, with exactly one fix-and-retest round.
...
- **Compound step** — the pre-ship agent (Shepherd's own inline compound doctrine): documents non-trivial solved-and-verified problems from the run under `docs/solutions/`. Runs before Ship so its docs commit rides the one push (a post-ship push would restart CI).
