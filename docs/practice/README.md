...
Shepherd's skill files are **doctrine skills**: they live in this repo under [`skills/`](../../skills/) and carry the planning and execution discipline as reusable doctrine that agents read before acting. Shepherd ships everything it needs; it depends on no external plugin. Skill paths in agent prompts resolve from the **session's starting directory**, not from a target repo (a plan or delivery can target a sibling checkout; the doctrine skills are always read from where the session began).
...
### Code-review fleet and inline tail-phase doctrine

Shepherd's Quality-phase code review runs on its **own** reviewer fleet — eight persona files under [`agents/`](../../agents/), dispatched via `agentType`, with no external dependency. The always-on quartet is `correctness-reviewer`, `maintainability-reviewer`, `testing-reviewer`, `standards-reviewer`. The conditional reviewers fire on risk surface or diff size: `adversarial-reviewer` (changedLines >= 50 OR plan has auth/payments risk surfaces), `security-reviewer` (auth/payments/crypto/public-api), `api-contract-reviewer` (public-api), `data-migration-reviewer` (migrations).

The deliver tail phases — simplify-as-you-go, the Quality simplify pass, the browser-proof Proof phase, the Compound learnings pass, and the commit/push/open-PR Ship phase — carry their doctrine **inline in the coordinator**: the rules are written at each dispatch site as the agent's prompt, not read from any installed SKILL.md. There is no plugin probe and no fallback path, because there is nothing external to fall back from.

Two format/protocol facts both halves agree on are likewise embedded directly in inline prompts: the **Shepherd plan format** (the plan-document section catalog the plan author writes to and the deliver parser reads) and the **execution/parse protocol** (parse conformance is the plan's release test; the per-task build follows it). The persona-selection trigger rules shepherd-plan uses to pick review lenses are copied inline into the classify-personas prompt.
