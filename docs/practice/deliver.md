...
| `repo` | session cwd | Target repository. When set, every agent dispatch is prefixed with a `TARGET REPOSITORY` grounding block so agents resolve all paths against it. |
...
- **Simplify-as-you-go**: after a wave that merged 2 or more tasks (and is not the final wave), an inline simplify agent (`SIMPLIFY_SCHEMA`) consolidates duplication on the integration branch via the installed `ce-simplify-code` skill BEFORE the next wave's worktrees fork from it. Behavior preservation is non-negotiable, and dead-code deletion requires a grep for remaining references; candidates it keeps are recorded in `kept` and surfaced as PR residuals. The final wave is covered by the Quality simplify instead.
...
Gated on `args.proof`, green validation, `agent-browser` availability, and budget (in that order; each failed gate sets a `skipped` proof object with the reason). When it runs, an inline proof agent (`sonnet`, `PROOF_SCHEMA`) browser-tests the affected routes via the installed `ce-test-browser` skill (`mode:pipeline`: port detection, dev-server auto-start, mapping changed files to routes, per-route checks with the `agent-browser` CLI). It returns `not-applicable` when no changed file maps to a route (it does not invent routes) and `tool-missing` when `agent-browser` is unusable.
