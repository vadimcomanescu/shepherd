...
Resolved during final review (recorded as decisions, not questions): v1 provenance is committed at `docs/plans/provenance/nadia-plan-v1/` (commit 795dd48, before this plan was authored) — the slimming pass reads mechanism rationale from there, not /tmp. The U2 → U1 dependency stands: both units own `workflows/nadia-plan.js` and `workflows/nadia-plan.test.mjs`, so the file-overlap invariant requires the dependency path; this is structural, not preference.
